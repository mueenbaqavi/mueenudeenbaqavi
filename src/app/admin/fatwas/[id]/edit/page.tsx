...
import { getAdminFatwaForEdit, listCategories } from "@/lib/admin-content-repository";
...
export default async function EditFatwaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [fatwa, categories] = await Promise.all([
    getAdminFatwaForEdit(id),
    listCategories("fatwa")
  ]);
  if (!fatwa) notFound();
  ...
  return (
    <>
      ...
      <section className="container py-10">
        <ContentEditorForm action={updateFatwaAction} initialValue={fatwa} kind="fatwa" categories={categories} />
      </section>
    </>
  );
}
